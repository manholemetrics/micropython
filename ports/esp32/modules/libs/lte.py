...
class LTE:
    ...
    def _receive_response(self, timeout_s : int) -> tuple[StatusCode, bytes]:
    ...
    def _setup(self, timeout_s: int = DEFAULT_TIMEOUT_S):
        ...
        self.wait_connection(timeout_s=timeout_s)

    UART : machine.UART | None = None

    def __init__(self, apn : str, bands : list[int], timeout_s : int = DEFAULT_TIMEOUT_S) -> None:
        self.apn = apn
        self.bands = bands
        self.uart = None
        self.p = None
        # wait for the modem to start
        self._setup(timeout_s)
    ...
    def check_connection(self) -> bool:
        ...
        while self.uart.any() > 0:
            for line in self.uart.read().decode("ascii").split("\r\n"):
                ...
                if 'CAMPED' in line:
                    camped += 1
                if 'STOPPED' in line or 'ERROR' in line or camped > 200:
                    self.stop_count += 1
    ...
    def http_get(self,
        ...
        resource : str = '/',
        timeout_s : int = 20,
        retry_count : int = 5) -> tuple[StatusCode, bytes]:
    ...
    def http_post(self,
        ...
        body : bytes | str,
        timeout_s : float = 90,
        retry_count : int = 3) -> tuple[StatusCode, bytes]:
